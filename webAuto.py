...
def addRow(wd: webdriver, dataList: list):
    ...
    wd.find_element(By.CSS_SELECTOR, anchor).click()
    wd.switch_to.active_element.send_keys(str(dataList[4]) + '\n')
    ...
    wd.find_element(By.CSS_SELECTOR, anchor).click()
    wd.switch_to.active_element.send_keys(str(dataList[-1]) + '\n')

    anchor = f'[data-index="0"]>div>div>div:nth-child(7)'
    wd.find_element(By.CSS_SELECTOR, anchor).click() 
    wd.switch_to.active_element.send_keys(str(dataList[9]) + '\n')

    anchor = f'[data-index="0"]>div>div>div:nth-child(8)'
    wd.find_element(By.CSS_SELECTOR, anchor).click()
    wd.switch_to.active_element.send_keys(str(dataList[10]) + '\n')

    anchor = f'[data-index="0"]>div>div>div:nth-child(9)'
    wd.find_element(By.CSS_SELECTOR, anchor).click()
    wd.switch_to.active_element.send_keys(str(dataList[11]) + '\n')
...
if __name__ == '__main__':
    ...
    dataList = ['0', '1', '', '北京市 | 北京市 | 清华大学', '2021', '理科', '671', '10', '是', '机航动大类车辆学院电子信息方向', 'test', '邮箱 guil21@qq.com', '2023-01-24 23: 00: 58', '2023-01-24 23: 04: 17', '3分19秒', '四川省', '达州市', '117.139.69.136', 'ChromiumEdge 109.0.1518.61', 'Windows 10', '681', '469']
    addRow(wd, dataList)
